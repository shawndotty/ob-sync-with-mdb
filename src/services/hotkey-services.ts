...
import { HotkeyConfig, HotkeyMapping, OBSyncWithMDBSettings } from "../types";
...
export class HotkeyService {
	...
	private OBSyncTemplateRootFolder;
	...
	private createSyncServiceHotkeys(): HotkeyDefinition[] {
		let syncServices: SyncServiceConfig[] = [];

		const syncConfigChecks = [
			{ id: "obSyncAirtable", name: "Airtable", key: "A" },
			{ id: "obSyncVika", name: "Vika", key: "V" },
			{ id: "obSyncFeishu", name: "Feishu", key: "F" },
		...
		for (const { id, name, key } of syncConfigChecks) {
			if (
				this.settings.updateIDs?.[
					id as keyof typeof this.settings.updateIDs
				]?.viewID
			) {
				syncServices.push({ name, key });
			}
		}
	...
	private createSyncTemplatePath(action: string, platform: string): string {
		return `${this.OBSyncTemplateRootFolder}/OBSyncDB/${action}/${platform}/IOTO-OB${action}${platform}.md`;
	}

	/**
	 * 添加IOTO热键
	 */
	async addOBSyncDBHotkeys(): Promise<void> {
		try {
			// 验证模板存在性
			...
			console.error(t("Add hotkeys error:"), error);
